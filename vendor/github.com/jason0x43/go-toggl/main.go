...
func (session *Session) DownloadDetailedReport(workspace int, since, until string) ([]byte, error) {
	params := map[string]string{
		"user_agent":   "jc-toggl",
		"grouping":     "projects",
		"since":        since,
	...
	data, err := session.get(ReportsAPI, "/details.pdf", params)
	if err != nil {
		return nil, err
	}
...
func (session *Session) GetProjects() (proj []Project, err error) {
	dlog.Printf("Listing projects")

	respData, err := session.get(TogglAPI, "/workspaces/1565620/projects", nil)
	if err != nil {
		return proj, err
	}

	err = json.Unmarshal(respData, &proj)
	if err != nil {
		return proj, err
	}
